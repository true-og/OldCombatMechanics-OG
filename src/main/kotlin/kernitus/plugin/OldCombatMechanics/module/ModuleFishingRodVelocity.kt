 ...
 *
 * Fishing rod gravity in 1.14+ is 0.03 while in 1.8 it is 0.04 Launch velocity in 1.9+ is also different from the 1.8
 * formula
 */
class ModuleFishingRodVelocity(plugin: OCMMain) : OCMModule(plugin, "fishing-rod-velocity") {
    ...
    fun onFishEvent(event: PlayerFishEvent) {
        ...
        var velocityX =
            -sin(playerYaw / 180.0f * Math.PI.toFloat()) *
                cos(playerPitch / 180.0f * Math.PI.toFloat()) *
                oldMaxVelocity
        var velocityZ =
        ...
        object : BukkitRunnable() {
                override fun run() {
                    ...
                    // We check both conditions as sometimes it's underwater but in seagrass, or when bobbing not
                    // underwater but the material is water
