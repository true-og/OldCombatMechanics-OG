...
class ModuleDisableOffHand(plugin: OCMMain) : OCMModule(plugin, "disable-offhand") {
    ...
    fun onInventoryClick(e: InventoryClickEvent) {
        ...
        } catch (ignored: NoSuchFieldError) {} // For versions below 1.16
        ...
        if (view.bottomInventory.type != InventoryType.CRAFTING && view.topInventory.type != InventoryType.CRAFTING)
            return
        ...
        if (
            currentItem != null &&
                currentItem.type == Material.SHIELD &&
        ...
        if (
            e.slot == OFFHAND_SLOT && // Let allowed items be placed into offhand slot with number keys (hotbar swap)
    ...
    fun onInventoryDrag(e: InventoryDragEvent) {
        val player = e.whoClicked
        if (
            !isEnabled(player) || e.inventory.type != InventoryType.CRAFTING || !e.inventorySlots.contains(OFFHAND_SLOT)
        )
            return
    ...
    override fun onModesetChange(player: Player) {
        ...
        if (isItemBlocked(offHandItem)) {
            ...
            if (inventory.addItem(offHandItem).isNotEmpty())
