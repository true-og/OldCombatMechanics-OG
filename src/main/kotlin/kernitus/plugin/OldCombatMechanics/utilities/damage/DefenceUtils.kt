...
/**
 * Utilities for calculating damage reduction from armour and status effects. Defence order is armour defence ->
 * resistance -> armour enchants -> absorption BASE -> HARD_HAT -> BLOCKING -> ARMOUR -> RESISTANCE -> MAGIC ->
 * ABSORPTION This class just deals with everything from armour onwards
 */
object DefenceUtils {
    private const val REDUCTION_PER_ARMOUR_POINT = 0.04
    private const val REDUCTION_PER_RESISTANCE_LEVEL = 0.2

    private val ARMOUR_IGNORING_CAUSES: MutableSet<DamageCause> =
        EnumSet.of(
    ...
    init {
        if (Reflector.versionIsNewerOrEqualTo(1, 11, 0)) ARMOUR_IGNORING_CAUSES.add(DamageCause.CRAMMING)
        if (Reflector.versionIsNewerOrEqualTo(1, 17, 0)) ARMOUR_IGNORING_CAUSES.add(DamageCause.FREEZE)
    }
    ...
    private val getAbsorptionAmount: SpigotFunctionChooser<LivingEntity, Any, Double> =
        SpigotFunctionChooser.apiCompatCall(
            ...
            { le: LivingEntity, _: Any? -> VersionCompatUtils.getAbsorptionAmount(le).toDouble() },
        )

    /**
     * Calculates the reduction by armour, resistance, armour enchantments and absorption. The values are updated
     * directly in the map for each damage modifier.
     *
    ...
    fun calculateDefenceDamageReduction(
        ...
        var currentDamage =
            damageModifiers.getOrDefault(DamageModifier.BASE, 0.0) +
        ...
        if (damageModifiers.containsKey(DamageModifier.ARMOR)) {
            ...
            if (
                !ARMOUR_IGNORING_CAUSES.contains(damageCause) &&
                    !(Reflector.versionIsNewerOrEqualTo(1, 19, 0) &&
                        damageCause == DamageCause.CONTACT &&
                        damagedEntity.location.block.type == Material.POINTED_DRIPSTONE)
            ) {
        ...
        if (damageCause != DamageCause.STARVATION) {
            ...
            if (
                damageModifiers.containsKey(DamageModifier.RESISTANCE) &&
            ...
            if (currentDamage > 0 && damageModifiers.containsKey(DamageModifier.MAGIC)) {
                val enchantsReductionFactor =
                    calculateArmourEnchantmentReductionFactor(
                        damagedEntity.equipment?.armorContents ?: emptyArray(),
                        damageCause,
                        randomness,
                    )
    ...
    fun getDamageAfterArmour1_8(
        ...
        for (i in armourContents.indices) {
            ...
            val slot = arrayOf(EquipmentSlot.FEET, EquipmentSlot.LEGS, EquipmentSlot.CHEST, EquipmentSlot.HEAD)[i]
            armourPoints +=
                getAttributeModifierSum(itemStack.type.getDefaultAttributeModifiers(slot)[Attribute.GENERIC_ARMOR])
        }
    ...
    /**
     * Applies all the operations for the attribute modifiers of a specific attribute. Does not take into account the
     * base value.
     */
    ...
    private enum class EnchantmentType(
        protection: Supplier<Set<DamageCause>>,
        private val typeModifier: Double,
        /**
        ...
        PROTECTION(
            Supplier {
                ...
                if (Reflector.versionIsNewerOrEqualTo(1, 10, 0)) damageCauses.add(DamageCause.HOT_FLOOR)
                if (Reflector.versionIsNewerOrEqualTo(1, 12, 0)) damageCauses.add(DamageCause.ENTITY_SWEEP_ATTACK)
                damageCauses
            },
            0.75,
            EnchantmentCompat.PROTECTION.get(),
        ),
        FIRE_PROTECTION(
            Supplier {
                val damageCauses = EnumSet.of(DamageCause.FIRE, DamageCause.FIRE_TICK, DamageCause.LAVA)
            ...
            1.25,
            EnchantmentCompat.FIRE_PROTECTION.get(),
        ),
        BLAST_PROTECTION(
            Supplier { EnumSet.of(DamageCause.ENTITY_EXPLOSION, DamageCause.BLOCK_EXPLOSION) },
            1.5,
            EnchantmentCompat.BLAST_PROTECTION.get(),
        ),
        PROJECTILE_PROTECTION(
            Supplier { EnumSet.of(DamageCause.PROJECTILE) },
            1.5,
            EnchantmentCompat.PROJECTILE_PROTECTION.get(),
        ),
        FALL_PROTECTION(Supplier { EnumSet.of(DamageCause.FALL) }, 2.5, EnchantmentCompat.FEATHER_FALLING.get());
