...
class ModuleSwordBlocking(plugin: OCMMain) : OCMModule(plugin, "sword-blocking") {
    ...
    fun onRightClick(e: PlayerInteractEvent) {
        ...
        if (e.isBlockInHand) {
            e.clickedBlock?.location?.let { location -> lastInteractedBlocks?.set(location, player.uniqueId) }
    ...
    @EventHandler fun onHotBarChange(e: PlayerItemHeldEvent) = restore(e.player, true)

    @EventHandler(priority = EventPriority.HIGHEST)
    fun onWorldChange(e: PlayerChangedWorldEvent) = restore(e.player, true)

    @EventHandler(priority = EventPriority.HIGHEST) fun onPlayerLogout(e: PlayerQuitEvent) = restore(e.player, true)
    ...
    fun onInventoryClick(e: InventoryClickEvent) {
        if (e.whoClicked is Player) {
            ...
            if (areItemsStored(player.uniqueId)) {
                ...
                if (
                    cursor != null && cursor.type == Material.SHIELD ||
    ...
    private fun restore(p: Player, force: Boolean = false) {
        ...
        if (!force && isPlayerBlocking(p)) scheduleRestore(p) else p.inventory.setItemInOffHand(storedItems.remove(id))
    }
    ...
    private fun scheduleRestore(p: Player) {
        ...
        val checkBlocking =
            Bukkit.getScheduler().runTaskTimer(plugin, Runnable { if (!isPlayerBlocking(p)) restore(p) }, 10L, 2L)
    ...
    private fun isPlayerBlocking(player: Player): Boolean {
        return player.isBlocking ||
            (versionIsNewerOrEqualTo(1, 11, 0) && player.isHandRaised && hasShield(player.inventory))
    }
