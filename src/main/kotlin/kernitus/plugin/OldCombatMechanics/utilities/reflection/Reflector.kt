...
object Reflector {
    ...
    fun getMethod(clazz: Class<*>, name: String, vararg parameterTypeSimpleNames: String): Method? {
        ...
        return allMethods
            .firstOrNull { method -> method.name == name && getParameterNames(method) == typeNames }
     ...
     * @return a function that invokes the retrieved cached method for its argument </R></U></T>
     */
    fun <T, U, R> memoiseMethodInvocation(clazz: Class<T>, name: String, vararg argTypes: String): (T, U) -> R {
                ...
                if (genericType is ParameterizedType) {
                    ...
                    if (
                        actualTypeArguments.size == 2 &&
                            actualTypeArguments[0] == keyType &&
                            actualTypeArguments[1] == valueType
                    ) {
                        field.isAccessible = true
                        return field
                    }
    ...
    fun getFieldValueByType(obj: Any, simpleClassName: String): Any {
        ...
        val matchingField =
            allFields.firstOrNull { field -> field.type.simpleName == simpleClassName }
    ...
    fun getConstructor(clazz: Class<*>, numParams: Int): Constructor<*>? =
        (clazz.declaredConstructors.asSequence() + clazz.constructors.asSequence())
            .firstOrNull { constructor -> constructor.parameterCount == numParams }
            ?.also { it.isAccessible = true }

    fun getConstructor(clazz: Class<*>, vararg parameterTypeSimpleNames: String): Constructor<*>? {
        val typeNames = parameterTypeSimpleNames.toList()
        return (clazz.declaredConstructors.asSequence() + clazz.constructors.asSequence())
            .firstOrNull { constructor -> getParameterNames(constructor) == typeNames }
            ?.also { it.isAccessible = true }
    }
     ...
     * @return True if `toCheck` somehow inherits from `inheritedClass`
     */
