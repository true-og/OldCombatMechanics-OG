...
class UpdateChecker(private val plugin: OCMMain) {
    ...
    private fun update(target: (String) -> Unit) {
        ...
        if (updater.isUpdateAvailable) {
            ...
            if (!autoDownload) {
                messages.add(
                    ChatColor.BLUE.toString() + "Click here to download it: " + ChatColor.GRAY + updater.updateURL
                )
            } else {
                messages.add(ChatColor.BLUE.toString() + "Downloading update: " + ChatColor.GRAY + updater.updateURL)
                try {
                    if (updater.downloadLatestVersion(plugin.server.updateFolderFile, plugin.file.name))
                        messages.add(
