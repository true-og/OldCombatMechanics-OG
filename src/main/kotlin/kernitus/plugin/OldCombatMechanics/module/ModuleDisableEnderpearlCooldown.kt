...
class ModuleDisableEnderpearlCooldown(plugin: OCMMain) : OCMModule(plugin, "disable-enderpearl-cooldown") {
    /**
     * Contains players that threw an ender pearl. As the handler calls launchProjectile, which also calls
     * ProjectileLaunchEvent, we need to ignore that event call.
     */
    ...
    fun onPlayerShoot(e: ProjectileLaunchEvent) {
        ...
        enderpearlItemStack = if (isEnderPearl(mainHand)) mainHand else if (isEnderPearl(offHand)) offHand else return
    ...
    fun getEnderpearlCooldown(playerUUID: UUID): Long {
        lastLaunched?.get(playerUUID)?.let { lastLaunchedValue ->
            ...
            return max(cooldownRemaining.toDouble(), 0.0)
