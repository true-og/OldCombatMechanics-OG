...
/** Reverts knockback formula to 1.8. Also disables netherite knockback resistance. */
class ModulePlayerKnockback(plugin: OCMMain) : OCMModule(plugin, "old-player-knockback") {
    ...
    fun onEntityDamage(event: EntityDamageEvent) {
        ...
        if (event.cause == DamageCause.ENTITY_ATTACK && event is EntityDamageByEntityEvent) {
    ...
    fun onEntityDamageEntity(event: EntityDamageByEntityEvent) {
        ...
        if (equipment != null) {
            ...
            if (bonusKnockback > 0) { // Apply bonus knockback
                playerVelocity.add(
                    Vector(
                        (-sin((damager.location.yaw * 3.1415927f / 180.0f).toDouble()) *
                            bonusKnockback.toFloat() *
                            knockbackExtraHorizontal),
                        knockbackExtraVertical,
                        cos((damager.location.yaw * 3.1415927f / 180.0f).toDouble()) *
                            bonusKnockback.toFloat() *
                            knockbackExtraHorizontal,
                    )
                )
