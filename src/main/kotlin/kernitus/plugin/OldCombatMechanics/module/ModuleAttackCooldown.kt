...
class ModuleAttackCooldown(plugin: OCMMain) : OCMModule(plugin, "disable-attack-cooldown") {
    ...
    @EventHandler(priority = EventPriority.HIGHEST) fun onPlayerLogin(e: PlayerJoinEvent) = adjustAttackSpeed(e.player)
    ...
    private fun adjustAttackSpeed(player: Player) {
        val attackSpeed = if (isEnabled(player)) module().getDouble("generic-attack-speed") else NEW_ATTACK_SPEED
