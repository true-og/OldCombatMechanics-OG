...
class ModuleGoldenApple(plugin: OCMMain) : OCMModule(plugin, "old-golden-apples") {
    ...
    companion object {
        ...
        private val gappleEffects: Set<PotionEffectType> =
            ImmutableSet.of(PotionEffectType.ABSORPTION, PotionEffectType.REGENERATION)

        // Napple: absorption IV, regen II, fire resistance I, resistance I
        private val nappleEffects: Set<PotionEffectType?> =
            ImmutableSet.of(
    ...
    override fun reload() {
        ...
        enchantedAppleRecipe =
            try {
                ShapedRecipe(NamespacedKey(plugin, "MINECRAFT"), ENCHANTED_GOLDEN_APPLE.newInstance())
            ...
            .runTaskLater(
                ...
                Runnable {
                    ...
                    player.activePotionEffects
                        .map { it.type }
                        .filter { defaultEffects.contains(it) }
        ...
        for (newEffect in newEffects) {
            ...
            if (highestExistingEffect != null) {
                ...
                if (newEffect.amplifier > highestExistingEffect.amplifier) {
                    ...
                    newEffect.amplifier == highestExistingEffect.amplifier &&
                        newEffect.duration > highestExistingEffect.duration
                ) {
                    target.addPotionEffect(newEffect)
                }
