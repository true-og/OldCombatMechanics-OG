...
object Config {
    ...
    private fun checkConfigVersion(): Boolean {
        val defaultConfig = YamlConfiguration.loadConfiguration(InputStreamReader(plugin.getResource(CONFIG_NAME)!!))
    ...
    fun reload() {
        ...
        ModuleLoader.modules.forEach(
            Consumer { module: OCMModule ->
                try {
                    module.reload()
                } catch (e: Exception) {
    ...
    private fun reloadModesets() {
        ...
        val modesetsSection =
            config.getConfigurationSection("modesets")
    ...
    private fun reloadWorlds() {
        ...
        val worldsSection =
            config.getConfigurationSection("worlds")
    ...
    /**
     * Checks whether the module is present in the default modeset for the specified world, or globally, is no world is
     * specified
     *
