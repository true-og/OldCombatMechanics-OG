...
- Disabled the plain Gradle `jar` task; `assemble` now produces only the shaded `shadowJar` output `build/libs/OldCombatMechanics.jar`, avoiding an unshaded duplicate `OldCombatMechanics-<version>.jar`.
...
- `ModuleOldBrewingStand` now keeps modern brewing-stand fuel slots permanently stocked with a synthetic single blaze-powder item, re-applies max fuel on open/close/click/drag/brew events, and cancels direct fuel-slot removal attempts while still feature-detecting legacy servers without a fuel API.
- `ModuleOldBrewingStand` no longer calls `InventoryView` accessors directly in runtime paths; it uses event/top-inventory APIs (`InventoryEvent#getInventory`, player inventory checks) so builds compiled against newer interface-based `InventoryView` remain compatible with 1.19.4 (where `InventoryView` is a class), preventing `IncompatibleClassChangeError` on inventory close/click/drag.
- `OldBrewingStandIntegrationTest` covers opening a brewing stand and direct fuel-slot clicks so the synthetic blaze powder remains present and the stand fuel stays full.
- `config.yml` now moves the built-in legacy combat modules into `always_enabled_modules`, leaving the default `old`/`new` modesets as empty compatibility placeholders so the old mechanics stay enabled regardless of player modeset.
