...
class ModuleAttackFrequency(plugin: OCMMain) : OCMModule(plugin, "attack-frequency") {
        ...
        Bukkit.getWorlds().forEach { world: World ->
            world.livingEntities.forEach { livingEntity: LivingEntity ->
                if (livingEntity is Player)
                    livingEntity.setMaximumNoDamageTicks(if (isEnabled(livingEntity)) playerDelay else DEFAULT_DELAY)
    ...
    @EventHandler fun onPlayerLogout(e: PlayerQuitEvent) = setDelay(e.player, DEFAULT_DELAY)
    ...
    fun onEntityTeleportEvent(e: EntityTeleportEvent) {
        ...
        if (fromWorld.uid !== toWorld.uid)
            entity.maximumNoDamageTicks = if (isEnabled(toWorld)) mobDelay else DEFAULT_DELAY
    }
}
