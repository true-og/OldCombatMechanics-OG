...
/**
 * Chooses a Spigot API function to use Chooses a function to apply based on a test supplier, remembers the choice and
 * only uses the corresponding function in the future.
 *
 * The branch to pick is determined during the *first execution of its [.apply] method!*. This means that no matter how
 * often the feature branch is invoked, it will never reconsider its choice.
 *
 ...
 * @param <R> the return type of the function </R></U></T>
 */
 ...
 */
(private val test: (T, U) -> Boolean, private val trueBranch: (T, U) -> R, private val falseBranch: (T, U) -> R) {
    ...
    fun interface ExceptionalFunction<T, U, R> {
        /**
         * This method is the target of the functional interface where you can write logic that might throw an
         * exception.
         *
         ...
         */
        @Throws(Throwable::class) fun applyWithException(t: T, u: U): R

        /**
         * Invokes [applyWithException]. If an exception is thrown, it wraps the exception into a [WrappedException].
         *
         * @param t the first function argument
         * @param u the second function argument
         * @return the function result
    ...
    companion object {
        ...
        private fun <T, U, R> onException(
            action: ExceptionalFunction<T, U, R>,
            success: (T, U) -> R,
            failure: (T, U) -> R,
        ...
        /**
         * Calls the Spigot API method if possible, otherwise uses reflection to access same method. Useful for API
         * methods that were only added after a certain version. Caches chosen method for performance.
         *
        ...
        fun <T, U, R> apiCompatReflectionCall(
            apiCall: ExceptionalFunction<T, U, R>,
            clazz: Class<T>,
            name: String,
         ...
         *
         * This should be used to avoid reflection wherever possible, making the plugin more compatible. Chosen method
         * is cached for performance. Do not use method references as they are eagerly-bound in Java 8.
         *
         * @param apiCall A reference to the function that should be called
