...
object WeaponDamages {
    ...
    val materialDamages: Map<Material, Double>
        get() {
            val materialMap: MutableMap<Material, Double> = EnumMap(org.bukkit.Material::class.java)
            damages.forEach { (name: String, damage: Double) ->
                val newName = name.replace("GOLD", "GOLDEN").replace("WOOD", "WOODEN").replace("SPADE", "SHOVEL")
