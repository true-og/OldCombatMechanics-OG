...
abstract class OCMModule protected constructor(protected var plugin: OCMMain, val configName: String) : Listener {
    ...
    /**
     * Returns if module should be enabled, giving priority to the attacker, if a human. If neither entity is a human,
     * checks if module should be enabled in the defender's world.
     *
    ...
    fun module(): ConfigurationSection {
        val section =
            plugin.config.getConfigurationSection(configName)
    ...
    /**
     * Called when player changes modeset. Re-apply any more permanent changes depending on result of isEnabled(player).
     *
    ...
    protected fun debug(text: String, sender: CommandSender) {
        if (debugEnabled()) {
            sendNoPrefix(sender, "&8&l[&fDEBUG&8&l][&f$moduleName&8&l]&7 $text")
        }
    }
    ...
    override fun toString(): String {
        return configName
            ...
            ?.joinToString(" ") { word -> word.lowercase().replaceFirstChar { it.uppercaseChar() } } ?: configName
    }
}
